...
function MasterMachineHaltDetailFun(myQuotasID, myType) {
    //alert("Halt" + myQuotasID + myType);
    $('#dlg_QuickMachineHaltDetail').dialog('open');
}
function TrendFun(myQuotasID, myType) {
    //alert("Trend" + myQuotasID + myType);
    ...
    grid = $('#grid_MasterMachineHaltInfo').datagrid({
        ...
        columns: [[{
            ...
            formatter: function (value, row, index) {
                var str = '';
                str = '<img class="iconImg" src = "/lib/extlib/themes/images/ext_icons/chart/chart_curve.png" title="趋势" onclick="TrendFun(\'' + row.QuotasID + '\',\'' + row.Type + '\');"/>';
                if (row["ShowDetail"] == "ShowDetail") {
                    str = str + '<img class="iconImg" src = "/lib/extlib/themes/images/ext_icons/zoom/zoom.png" title="详细信息" onclick="MasterMachineHaltDetailFun(\'' + row.QuotasID + '\',\'' + row.Type + '\');"/>';
                }
                return str;
            }
        }]],
...
function loaddQuickTrendDialog() {
